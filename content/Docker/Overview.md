...
## [Installation 安装]({{< ref "/Docker/Overview#_installation" >}})
...
## [Editing Docker files 编辑 Docker 文件]({{< ref "/Docker/Overview#_editing-docker-files" >}})

You can get [IntelliSense]({{< ref "/UserGuide/IntelliSense" >}}) by clicking Ctrl+Space when editing your `Dockerfile` and `docker-compose.yml` files, with completions and syntax help for common commands.
...
## [Generating Docker files 生成 Docker 文件]({{< ref "/Docker/Overview#_generating-docker-files" >}})
...
## [Docker Explorer Docker 资源管理器]({{< ref "/Docker/Overview#_docker-explorer" >}})
...
## [Docker commands Docker 命令]({{< ref "/Docker/Overview#_docker-commands" >}})
...
## [Docker Compose]({{< ref "/Docker/Overview#_docker-compose" >}})
...
## [Using image registries 使用映像注册表]({{< ref "/Docker/Overview#_using-image-registries" >}})
...
An image in an Azure Container Registry can be deployed to Azure App Service or Azure Container Apps directly from VS Code. See [Deploy to Azure]({{< ref "/Docker/DeploytoAzure" >}}) to get started. For more information about how to authenticate to and work with registries, see [Using container registries]({{< ref "/Docker/Registries" >}}).

​​	Azure 容器注册表中的图像可以直接从 VS Code 部署到 Azure 应用服务或 Azure 容器应用。请参阅部署到 Azure 以开始。有关如何向注册表进行身份验证并使用注册表工作的详细信息，请参阅使用容器注册表。

## [Debugging services running inside a container 调试在容器内运行的服务]({{< ref "/Docker/Overview#_debugging-services-running-inside-a-container" >}})

You can debug services built using .NET (C#) and Node.js that are running inside a container. The extension offers custom tasks that help with launching a service under the debugger and with attaching the debugger to a running service instance. For more information, see [Debug containerized apps]({{< ref "/Docker/Debug" >}}) and [Customize the Docker extension]({{< ref "/Docker/Customize" >}}).

​​	您可以调试在容器内运行的、使用 .NET (C#) 和 Node.js 构建的服务。该扩展提供自定义任务，帮助在调试器下启动服务，并将调试器附加到正在运行的服务实例。有关详细信息，请参阅调试容器化应用和自定义 Docker 扩展。

## [Azure CLI integration Azure CLI 集成]({{< ref "/Docker/Overview#_azure-cli-integration" >}})
...
## [Next steps 后续步骤]({{< ref "/Docker/Overview#_next-steps" >}})
