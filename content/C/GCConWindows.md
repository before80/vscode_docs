...
​​​	在本教程中，您将配置 Visual Studio Code 以使用 mingw-w64 中的 GCC C++ 编译器 (g++) 和 GDB 调试器来创建可在 Windows 上运行的程序。配置 VS Code 后，您将编译、运行和调试一个 Hello World 程序。
...
​​​	本教程不教您有关 GCC、GDB、minGW-w64 或 C++ 语言的知识。对于这些主题，网上有很多很好的资源可用。
...
1. You can download the latest installer from the MSYS2 page or use this [**direct link to the installer**](https://github.com/msys2/msys2-installer/releases/download/2023-05-26/msys2-x86_64-20230526.exe).
   ...
   ​​​	您可以从 MSYS2 页面下载最新的安装程序，或使用此直接链接到安装程序。
...
3. In the wizard, choose your desired Installation Folder. Record this directory for later. In most cases, the recommended directory is acceptable. The same applies when you get to setting the start menu shortcuts step. When complete, ensure the **Run MSYS2 now** box is checked and select **Finish**. This will open a MSYS2 terminal window for you.
   ...
   ​​​	在向导中，选择您想要的安装文件夹。记录此目录以备后用。在大多数情况下，推荐的目录是可以接受的。在设置开始菜单快捷方式步骤时也是如此。完成后，确保选中立即运行 MSYS2 框，然后选择完成。这将为您打开一个 MSYS2 终端窗口。
...
5. Accept the default number of packages in the `toolchain` group by pressing Enter.
   ...
   ​​​	通过按 Enter 键接受 `toolchain` 组中的默认软件包数。
...
7. Add the path to your MinGW-w64 `bin` folder to the Windows `PATH` environment variable by using the following steps:
   ...
   ​​​	通过以下步骤将路径添加到 Windows `PATH` 环境变量中的 MinGW-w64 `bin` 文件夹：
...
​​​	您应该会看到输出，其中指出您已安装的 GCC、g++ 和 GDB 的版本。如果不是这种情况：
...
​​​	在您完成本教程时，您将在工作区中的 `.vscode` 文件夹中看到创建的三个文件：
...
​​​	请记住，C++ 扩展使用您已安装在计算机上的 C++ 编译器来构建您的程序。在尝试在 VS Code 中运行和调试 `helloworld.cpp` 之前，请确保您已完成“安装 MinGW-w64 工具链”步骤。
...
​​​	首次运行程序时，C++ 扩展会创建一个 `tasks.json` 文件，您可以在项目的 `.vscode` 文件夹中找到它。 `tasks.json` 存储您的构建配置。
...
​​​	此任务告诉 g++ 获取活动文件 ( `${file}` )，编译它，并在当前目录 ( `${fileDirname}` ) 中创建一个输出文件 ( `-o` 开关)，其名称与活动文件相同，但扩展名为 `.exe` ( `${fileBasenameNoExtension}.exe` )。对我们来说，这会导致 `helloworld.exe` 。
...
​​​	值 `problemMatcher` 选择输出解析器，用于在编译器输出中查找错误和警告。对于 GCC，如果您使用 `$gcc` 问题匹配器，您将获得最佳结果。
...
​​​	从现在开始，播放按钮将从 `tasks.json` 读取，以了解如何构建和运行您的程序。您可以在 `tasks.json` 中定义多个构建任务，而标记为默认的任务将由播放按钮使用。如果您需要更改默认编译器，您可以在命令面板中运行任务：配置默认构建任务。或者，您可以修改 `tasks.json` 文件，并通过替换此段来删除默认值：
...
​​​	然后，您将看到一个用于各种预定义调试配置的下拉菜单。选择 C/C++：g++.exe 构建并调试活动文件。

![C++ debug configuration dropdown](https://code.visualstudio.com/assets/docs/cpp/playbutton/select-gcc-compiler.png)

VS Code creates a `launch.json` file in the `.vscode` folder`, which looks something like this:

​​​	VS Code 在 `.vscode` 文件夹中创建一个 `launch.json` 文件，它看起来像这样：
...
​​​	在此，我们将配置名称更改为 GCC，将编译器路径下拉菜单设置为 g++ 编译器，并将 IntelliSense 模式设置为与编译器 (gcc-x64) 匹配。
...
​​​	Windows 机器上的 UCRT 仅包含在 Windows 10 或更高版本中。如果您使用的是其他版本的 Windows，请运行不使用 UCRT 的以下命令：
