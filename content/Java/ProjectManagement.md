...
&zeroWidthSpace;Java 扩展的项目管理提供了一个用户界面，用于配置非托管文件夹的类路径。可以在类路径配置页面中手动设置类路径。可以通过从命令面板 (Ctrl+Shift+P) 执行 Java: 配置类路径命令来打开它。
...
&zeroWidthSpace;对于 Maven 项目，可以通过单击项目视图中 Maven 依赖项节点旁边的 + 图标来添加依赖项。
