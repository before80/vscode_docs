...
> 原文: [https://code.visualstudio.com/docs/getstarted/settings](https://code.visualstudio.com/docs/getstarted/settings)
...
​​	您可以通过 Visual Studio Code 的各种设置对其进行自定义配置。VS Code 编辑器、用户界面和功能行为的几乎每个部分都有可以修改的选项。
...
​​	VS Code 为设置提供了几个不同的范围。当您打开工作区时，您至少会看到以下两个范围：
...
​​	要修改用户设置，您将使用设置编辑器来查看和更改 VS Code 设置。

To open the Settings editor, navigate to **File** > **Preferences** > **Settings**.

​​	要打开设置编辑器，请导航至文件 > 首选项 > 设置。

You can also open the Settings editor from the **Command Palette** (Ctrl+Shift+P) with **Preferences: Open Settings** or use the keyboard shortcut (Ctrl+,).

​​	您还可以通过命令面板 (Ctrl+Shift+P) 使用“首选项：打开设置”或使用键盘快捷键 (Ctrl+,) 来打开“设置”编辑器。
...
​​	打开“设置”编辑器后，您可以搜索并查找您要查找的设置。当您使用搜索栏进行搜索时，它不仅会显示并突出显示与您的条件匹配的设置，还会过滤掉那些不匹配的设置。这使得查找设置变得快速而轻松。

![Filtering settings by searching in the Settings editor](./Settings_img/settings-search.png)

In the example below, the Side Bar location and file icon theme have been changed.

​​	在下面的示例中，已更改侧边栏位置和文件图标主题。

![Modified settings in the Settings editor showing blue vertical bars to the left of them](./Settings_img/settings-modified.png)
...
​​	当您更改设置时，VS Code 会应用对设置的更改。已修改的设置会用蓝色线条指示，类似于编辑器中已修改的行。

The gear icon (**More Actions...** Shift+F9) opens a context menu with options to reset the setting to its default value as well as copy the setting ID or JSON name-value pair.

​​	齿轮图标（更多操作... Shift+F9）会打开一个上下文菜单，其中包含将设置重置为其默认值以及复制设置 ID 或 JSON 名称-值对的选项。
...
​​	每个设置都可以通过复选框、输入或下拉菜单进行编辑。编辑文本或选择您想要更改为所需设置的选项。
...
​​	设置以组的形式表示，以便您可以轻松导航它们。顶部有一个“常用”组，其中显示了流行的自定义项。

Below, the Source Control settings are focused by selecting **Source Control** in the tree view.

​​	在下方，通过在树视图中选择源代码管理，源代码管理设置获得焦点。

![Settings editor with the Source Control section of the table of contents selected](./Settings_img/settings-groups.png)
...
​​	例如，让我们从 VS Code 中隐藏活动栏。活动栏是左侧的宽边框，其中包含各种图标，用于不同的视图，如文件资源管理器、搜索、源代码管理和扩展。您可能希望隐藏活动栏，以便为编辑器腾出更多空间，或者如果您更喜欢通过视图菜单或命令面板打开视图。

![Activity Bar display on the left of the editor](./Settings_img/activity-bar.png)

Open the Settings Editor (Ctrl+,) and type "activity" in the Search bar. You should see at least five settings.

​​	打开设置编辑器 (Ctrl+,)，然后在搜索栏中键入“activity”。您应该会看到至少五个设置。

![Settings editor with 'activity' in the Search bar with at least five settings](./Settings_img/search-for-activity.png)
...
​​	您可以进一步将范围限制为左侧目录中的外观组下的那些设置。现在应该只有三个设置。
...
​​	您现在可以选中和取消选中工作台>活动栏：可见设置以隐藏和取消隐藏活动栏。请注意，当您将设置值更改为不同于默认值时，您会在左侧看到一条蓝线。

![Activity Bar: Visible unchecked and Activity Bar is hidden](./Settings_img/activity-bar-hidden.png)
...
​​	要检查您已配置哪些设置，搜索栏中有一个 `@modified` 筛选器。如果设置的值与默认值不同，或者如果其值在各自的设置 JSON 文件中明确设置，则该设置将显示在此筛选器下。如果您忘记是否配置了某个设置，或者由于您意外配置了某个设置而导致编辑器未按预期工作，则此筛选器可能很有用。
...
​​	还有几个其他方便的筛选器，可帮助您搜索设置。

![Setting editor @ tag filter dropdown](./Settings_img/settings-editor-filters.png)
...
- `@ext` - Settings specific to an extension. You provide the extension ID such as `@ext:ms-python.python`.
  `@ext` - 特定于扩展程序的设置。您提供扩展程序 ID，例如 `@ext:ms-python.python` 。
...
​​	搜索栏会记住您的设置搜索查询，并支持撤消/重做 (Ctrl+Z/Ctrl+Y)。您可以使用搜索栏右侧的“清除设置搜索输入”按钮快速清除搜索词或筛选器。
...
​​	已安装的 VS Code 扩展也可以贡献其自己的设置，您可以在“设置”编辑器的“扩展”部分下查看这些设置。

![C++ extension settings in the Settings editor](./Settings_img/cpp-extension-settings.png)
...
​​	您还可以通过选择扩展并查看“功能贡献”选项卡，从“扩展”视图 (Ctrl+Shift+X) 查看扩展的设置。

![Python extension Settings list under Feature Contributions tab](./Settings_img/python-feature-contributions.png)

Extension authors can learn more about adding custom settings in the [configuration contribution point documentation](https://code.visualstudio.com/api/references/contribution-points#_contributes.configuration).
...
​​	设置编辑器是允许您审阅和修改存储在 `settings.json` 文件中的设置值的 UI。您可以通过使用“首选项：在编辑器中以 JSON 格式直接编辑用户设置”这一指令在编辑器中直接将其开启来审阅和编辑此文件。设置通过 JSON 编写，方法是明确设置 ID 和值。 `settings.json` 文件对设置和值提供完整的 IntelliSense，包括可提供信息的悬停和可进行的联想。还会突出显示因设置名称或 JSON 格式不正确而产生的错误。有些设置只能在 `settings.json` 中编辑，如工作台：色彩自定义，并在设置编辑器中显示“在 settings.json 中编辑”的超文本。更改 settings.json举个例子，让我们来更改编辑器行号的颜色。单击“在 settings.json 中编辑”的超文本，并添加以下 JSON：现在，`settings.json` 文件中的编辑器行号为绿色。删除 `settings.json` 的设置代码块，可将行号的颜色恢复为初始设置。注意：上面的示例会更改所有色彩主题的编辑器行号，但您可针对特定的色彩主题调整色彩，甚至创建自己的色彩主题扩展。

![User settings.json open in the editor](./Settings_img/settings-json-in-editor.png)
...
![IntelliSense for settings.json open in the editor](./Settings_img/settings-json-intellisense.png)
...
![settings.json editor with green line numbers](./Settings_img/color-customization-example.png)
...
​​	您可以通过“设置”编辑器工作区选项卡进行编辑，或使用“首选项：打开工作区设置”命令直接打开该选项卡。

![Settings editor with Workspace tab highlighted](./Settings_img/settings-editor-workspace-tab.png)
...
​​	与用户设置类似，工作区设置也存储在 `settings.json` 文件中，您可以通过“首选项：打开工作区设置 (JSON)”命令直接编辑该文件。
...
​​	自定义特定于语言的设置的一种方法是打开“设置”编辑器，点击筛选器按钮，然后选择语言选项以添加语言筛选器。或者，也可以直接在搜索小组件中键入 `@lang:languageId` 形式的语言筛选器。显示的设置将可针对该特定语言进行配置，并将显示特定于该语言的设置值（如果适用）。
...
​​	在存在语言筛选器的情况下修改设置时，将在给定范围内针对该语言配置设置。例如，在搜索小组件中存在 `@lang:css` 筛选器的情况下修改用户范围 `diffEditor.codeLens` 设置时，“设置”编辑器会将新值保存到用户设置文件的 CSS 特定部分。

![Editing the CSS-specific user-scoped diffEditor.codeLens setting in the Settings editor](./Settings_img/settings-css-example.png)
...
>
> ​​	注意：如果在搜索小组件中输入多个语言筛选器，当前行为是仅使用第一个语言筛选器。

Another way to customize your editor by language is by running the global command **Preferences: Configure Language Specific Settings** (command ID: `workbench.action.configureLanguageBasedSettings`) from the **Command Palette** (Ctrl+Shift+P) which opens the language picker. Select the language you want. Then, the Settings editor opens with a language filter for the selected language, which allows you to modify language-specific settings for that language. Though, if you have the `workbench.settings.editor` setting set to `json`, then the `settings.json` file opens with a new language entry where you can add applicable settings.

​​	另一种通过语言自定义编辑器的方法是通过命令面板（Ctrl+Shift+P）运行全局命令首选项：配置特定于语言的设置（命令 ID： `workbench.action.configureLanguageBasedSettings` ），这将打开语言选择器。选择所需的语言。然后，设置编辑器将打开一个针对所选语言的语言筛选器，允许您修改该语言的特定于语言的设置。不过，如果您将 `workbench.settings.editor` 设置设置为 `json` ，那么 `settings.json` 文件将打开一个新的语言条目，您可以在其中添加适用的设置。

![Configure language-specific settings command typed up in the Command Palette](./Settings_img/pref-config-lang-settings.png)

Select the language via the dropdown:

​​	通过下拉菜单选择语言：

![Select language dropdown](./Settings_img/lang-selection.png)

Now you can start editing settings specifically for that language:

​​	现在，您可以开始编辑专门针对该语言的设置：

![Settings editor showing a specific language filter](./Settings_img/lang-based-settings-editor.png)

Or, if `workbench.settings.editor` is set to `json`, now you can start adding language-specific settings to your user settings:

​​	或者，如果 `workbench.settings.editor` 设置为 `json` ，现在您可以开始向用户设置中添加特定于语言的设置：

![Suggestions for language-specific settings shown in the settings JSON file](./Settings_img/lang-based-settings.png)
...
​​	如果您已打开一个文件并希望自定义此文件类型的编辑器，请选择 VS Code 窗口右下角的状态栏中的“语言模式”。这将打开“语言模式”选择器，其中包含一个选项“配置‘language_name’语言的基于设置”。选择此选项将打开您的用户 `settings.json` ，其中包含语言条目，您可以在其中添加适用的设置。
...
​​	特定于语言的编辑器设置始终会覆盖非特定于语言的编辑器设置，即使非特定于语言的设置具有更窄的范围。例如，特定于语言的用户设置会覆盖非特定于语言的工作区设置。
...
​​	您可以通过将特定于语言的设置放在工作区设置中（就像其他设置一样）来将它们限定到工作区。如果您在用户和工作区范围内为同一种语言定义了设置，那么它们将合并，优先考虑在工作区中定义的设置。
...
​​	您可以同时为多种语言配置特定于语言的编辑器设置。以下示例演示如何在 `settings.json` 文件中同时自定义 `javascript` 和 `typescript` 语言的设置：
...
​​	具有基本类型和数组类型的数值会被覆盖，这意味着优先于另一个作用域的作用域中配置的值将用于替换另一个作用域中的值。但是，具有对象类型的数值会进行合并。
...
​​	例如， `workbench.colorCustomizations` 采用一个指定一组 UI 元素及其所需颜色的对象。如果您的用户设置将编辑器背景设置为蓝色和绿色：
...
​​	结果是，当该工作区打开时，这两个颜色自定义项会组合在一起，就像您指定了以下内容一样：
...
​​	某些设置允许您指定 VS Code 将运行的可执行文件来执行某些操作。例如，您可以选择集成终端应使用哪个 shell。为了增强安全性，此类设置只能在用户设置中定义，而不能在工作区范围内定义。
...
​​	以下是 Visual Studio Code 的默认设置及其值。您还可以在“设置”编辑器中查看默认值，或通过“首选项：打开默认设置 (JSON)”查看 `defaultSettings.json` 的只读版本。
